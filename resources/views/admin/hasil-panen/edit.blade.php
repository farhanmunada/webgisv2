<x-admin-layout>
    ...
    <div class="max-w-2xl">
        <div class="mb-8 flex items-center justify-between">
            <a href="{{ route('admin.hasil-panen.index') }}" class="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-500 hover:text-[#8B4513] transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Kembali ke Daftar
            </a>
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">ID: #{{ $hasil_panen->id }}</span>
        </div>
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-5 sm:p-8 border-b border-slate-50">
                <h3 class="text-lg font-black text-slate-900">Perbarui Data Produksi</h3>
                <p class="text-slate-500 text-xs mt-1">Lakukan penyesuaian angka produksi untuk wilayah {{ $hasil_panen->kecamatan }}.</p>
            </div>
            
            <form method="POST" action="{{ route('admin.hasil-panen.update', $hasil_panen) }}" class="p-5 sm:p-8 space-y-4 sm:space-y-6">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Wilayah Kecamatan</label>
                    <input type="text" name="kecamatan" value="{{ old('kecamatan', $hasil_panen->kecamatan) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="hasil_robusta" value="{{ old('hasil_robusta', $hasil_panen->hasil_robusta) }}" required
                            class="w-full px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl focus:ring-4 focus:ring-[#8B4513]/5 focus:border-[#8B4513] outline-none transition-all font-semibold text-slate-700">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="hasil_arabika" value="{{ old('hasil_arabika', $hasil_panen->hasil_arabika) }}" required
                            class="w-full px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl focus:ring-4 focus:ring-[#8B4513]/5 focus:border-[#8B4513] outline-none transition-all font-semibold text-slate-700">
                ...
                <div class="pt-4 flex flex-col gap-3">
                    ...
                    <a href="{{ route('admin.hasil-panen.index') }}" class="text-center py-3 text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Batal & Kembali</a>
                </div>
            </form>
        </div>
    </div>
