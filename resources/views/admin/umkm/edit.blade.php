        ...
        <form action="{{ route('admin.umkm.update', $umkm) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="alamat" rows="3" required
                                class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] outline-none transition-all font-semibold text-slate-700">{{ old('alamat', $umkm->alamat) }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Deskripsi (Opsional)</label>
                            <textarea name="deskripsi" rows="5"
                                class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] outline-none transition-all font-semibold text-slate-700">{{ old('deskripsi', $umkm->deskripsi) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-5 sm:p-8 sticky top-8">
                    ...
                    <a href="{{ route('admin.umkm.index') }}" class="block text-center py-4 text-xs sm:text-sm font-bold text-slate-400 hover:text-slate-600 transition-all mt-2">Batal & Kembali</a>
                </div>
            </div>
        </form>
    </div>
