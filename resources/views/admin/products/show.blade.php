<x-admin-layout>
    ...
    <div class="max-w-4xl bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="md:flex">
            <div class="md:w-1/3">
            ...
            <div class="p-8 md:w-2/3">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h3 class="text-2xl font-black text-slate-900">{{ $product->nama_produk }}</h3>
                        <p class="text-[#8B4513] font-bold mt-1">{{ $product->umkm->nama_umkm ?? 'UMKM Tidak Diketahui' }}</p>
                    </div>
                    <div class="text-2xl font-black text-[#8B4513]">
                ...
                <div class="pt-8 border-t border-slate-50 flex justify-between items-center">
                    ...
                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('Hapus produk ini?')">
                        @csrf @method('DELETE')
                        <button class="px-6 py-2 bg-rose-50 text-rose-600 rounded-xl font-bold hover:bg-rose-100 transition-colors">
                            Hapus Produk
                        </button>
                    </form>
                </div>
